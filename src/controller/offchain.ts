...
async function fetcher(urlString: string) {
      return await fetch(urlString).then((response) => response.text())
            .then(async (body) => {
                  ...
                  let latestHistorys = await keyVal.find({key:"LatestHistoryFetch" })
                  if (latestHistorys.length == 0){
                          latestHistory = new keyVal({ key: "LatestHistoryFetch", value: history })
                  }else{
                  ...
                  var HistoryErrors = await keyVal.find({ key: "LatestHistoryError" });
                  if (HistoryErrors.length == 0){
                        latestHistoryError = new keyVal({ key: "LatestHistoryError", value: { time: time, error: error } })
                  }else{
...
export async function confirmTransfer(ticketId: string) {
      ...
      try {
            ...
            if (fetchData?.status == 'error') {
                  ...
                  events = JSON.parse(JSON.stringify(fetchData?.data)).events;
